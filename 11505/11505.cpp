...
void change(long long a, long long b, int n) {
    ...
    for (int i=0; i<getH(n)+1; i++) {
        now = (now-1)/2;
        nodes[now].val = (nodes[now*2+1].val * nodes[now*2+2].val) % 1000000007;
    }
}
...
long long getValue(int from, int to, int nodeNum) {
    ...
    if (from<=mid && to>mid) {
        return (getValue(from, mid, nodeNum*2+1) * getValue(mid+1, to, nodeNum*2+2))%1000000007;
    } else if (to<=mid) {
...
int main(void) {
    ...
    for (int i=getH(N)-1; i>=0; i--) {
        for (int j=0; j<powTwo(i); j++) {
            nodes[powTwo(i)-1+j].val=(nodes[(powTwo(i)-1+j)*2+1].val*nodes[(powTwo(i)-1+j)*2+2].val)%1000000007;
